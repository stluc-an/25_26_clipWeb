...
class Animator extends Drawable {
	...
	constructor(counter, imgLength, addressPattern, token="_imgNum_"){
		super();
		this.imgs = [];
		this.cursor = 0;
		this.pointer = 0;
		...
		while(counter <= imgLength){
			let imgAddress = addressPattern.replace(token, counter)
	...
	next(hasToLoop=false){
		this.pointer++;
		if(this.pointer >= this.imgs.length){
			this.pointer = hasToLoop?0:this.imgs.length-1;
		}
	}
}